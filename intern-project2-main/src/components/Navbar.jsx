    ...
    <div>
      <div className='py-2 sm:px-4 px-2 flex lg:flex-row flex-col justify-between'>
        <div className='lg:justify-between sm:justify-start flex sm:flex-row flex-col  items-center space-x-12'>
          ...
          <img src={G_logo} className='lg:w-[100px] max-md:hidden   sm:w-[90px] w-[150px]  ' />
        </div>
        <div className='flex sm:flex-row relative flex-col-reverse  max-sm:mt-2 justify-between xl:space-x-10 sm:mt-[5px] lg:mt-0 sm:items-center'>
          ...
          <div className='flex flex-row relative items-center justify-center space-x-1 '>
            ...
            <input className='py-[6px] px-2  max-sm:w-[300px] border-green-600 rounded-[20px] border-2 cursor-not-allowed' value={searchval}  onChange={(e)=>{setsearch(e.target.value)}} disabled={true} />
            <button className='bg-[#168816] xl:px-7 sm:px-5 px-6  py-3 rounded-[22px] border-white border-2 shadow-lg cursor-not-allowed'><BsSearch className='text-[white]' /></button>
          </div>
        </div>
      </div>
      ...
      <div className='lg:hidden pb-2 '>
        ...
        <div className={`${toggle ? 'flex' : 'hidden'}  bg-[#7ead5e] my-2 w-full sidebar transition-all`}>
          <ul className='list-none flex flex-col justify-center  flex-1'>
           <NavLink to='/' onClick={tog}> <li className=' cursor-pointer  p-2 border-t-[#5f7d4b] border-t hover:bg-[#008000]'> <AiOutlineHome className='text-yellow-300 text-2xl' /></li></NavLink>
           
    
            <NavLink to='aboutblacksalt' onClick={tog}> <li className=' cursor-pointer  font-serif  p-2 border-t-white border-t hover:bg-[#008000] text-white' >About</li></NavLink>
           ...
           <NavLink to='general-info' onClick={tog}><li className=' cursor-pointer font-serif  p-2 border-t-white border-t hover:bg-[#008000] text-white'>General Info</li></NavLink> 
            <li onClick={tog} className=' cursor-pointer font-serif hover:bg-[#008000]  p-2 border-t-white border-t text-white'><a href={Restration}>Registration Process</a></li>
          </ul>
        </div>
      </div>
    </div>
